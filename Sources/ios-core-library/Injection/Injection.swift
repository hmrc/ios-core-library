...
extension MobileCore.Injection {
    public class Injector: ResettableInjector, CustomStringConvertible {
        ...
        required public init(_ identifier: String, _ createStandard: @escaping Create) {
            ...
            MobileCore.Injection.injectors[identifier] = self
        }
        ...
        @discardableResult public func inject(_ object: Any?=nil) -> Self {
            ...
            if let existing = _obj {
                if shouldLog { Log.info(message: "Replacing Existing Injection object: \(existing)") }
                if let disposable = existing as? DisposableResourceOwner {
                    if shouldLog { Log.info(message: "Disposable Object \(disposable)") }
    ...
    public static func reset() {
        MobileCore.Injection.injectors.forEach { (injectorName, injector) in
            print("Resetting Injector \(injectorName)")
            injector.reset()
        }
        MobileCore.Injection.initialise()
    }
}
