...
extension UIView {
    ...
    @discardableResult
    public func descendantButtonWith(text: String, allowPartialMatch: Bool=false) -> UIButton? {
        if let self = self as? UIButton {
            if allowPartialMatch {
                if self.currentTitle?.contains(text) ?? false { return self }
            } else if self.currentTitle == text { return self }
        } else {
            for subview in subviews {
                if let label = subview.descendantButtonWith(text: text, allowPartialMatch: allowPartialMatch) {
                    return label
                }
            }
        }
